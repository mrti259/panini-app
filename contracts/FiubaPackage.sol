...
interface FiubaStickerInterface {
...
contract FiubaPackage is Ownable {
    ...
    function setStickersPerPackage(uint256 _amount) external onlyOwner {
        require(_amount > 0);
    ...
    function buyPackages(uint256 _quantity) external withPriceSetted {
    ...
    function openPackage() external {
        ...
        FiubaStickerInterface(stickerAddress).createStickers(
            msg.sender,
            stickersPerPackages
        );
    ...
    function getAmountOfPackagesFrom(address _owner)
        external
        view
        returns (uint256)
    {
