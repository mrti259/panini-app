...
contract FiubaExchange is FiubaSticker {
    struct exchange {
        address owner;
    ...
    function iWantToExchange(uint256 tokenId, uint256 playerId)
        public
        ...
        isActive(exchangeId)
    {
        ...
        (bool wantedPlayer, uint256 tokenIdWanted) = hasWantedPlayer(
            playerIdWanted,
            interested
        );
        require(
            wantedPlayer,
            "The person who is accepting exchange hasn't wanted player"
        );
    ...
    function cancelExchangeWithExchangeId(uint256 tokenId) private {
    ...
    function hasWantedPlayer(uint256 playerIdWanted, address wallet)
        internal
        view
